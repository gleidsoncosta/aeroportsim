...
class Router (object):
    def __init__(self, env, numMicros, latencia, conexao, requisicaoHTTP, requestPerSecond):
        ...
        self.conexao = simpy.Resource(conexao)
...
class WebService (object):
    ...
    # A resposta deve ser um dos tamanhos de requicições pela taxa de transferencia + o RTT (Round Trip Time)
    def reply (self):
        yield self.env.timeout(random.choice(self.tamanhoRequisicao) * self.taxaTransferencia + self.RTT)

def request (env, router, requestID, webService):
...
def setup (env):
    ...
    for i in range (4):
        env.process (request(env, router, i, webService))
